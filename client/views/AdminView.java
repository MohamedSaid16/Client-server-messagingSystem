...
private JPanel createMessagingTab() {
    ...
    sendAnnouncementButton = createStyledButton("📢 Send Announcement", Color.WHITE, LIGHT_BLUE);
    sendAnnouncementButton.addActionListener(e -> showAnnouncementDialog());
    quickActionPanel.add(sendAnnouncementButton);
    
    JButton clearMessagesButton = createStyledButton("🗑️ Clear Messages", Color.WHITE, LIGHT_BLUE);
    ...
    filterCombo.addActionListener(e -> filterMessages((String)filterCombo.getSelectedItem()));
...
private void initializeMessaging() {
    ...
    int adminId = Integer.parseInt(userInfo.get("userId"));
    adminController.subscribeToAdminMessages(listener, adminId);
}
...
private void showMessageNotification(Message message) {
    // Show system tray notification or dialog
    JOptionPane.showMessageDialog(this,
        message.getContent(),
...
private void showAnnouncementDialog() {
    JDialog dialog = new JDialog(this, "Send Announcement", true);
    dialog.setLayout(new BorderLayout());
    dialog.setSize(500, 400);
    ...
    JComboBox<String> recipientCombo = new JComboBox<>(
        new String[]{"All Users", "Students", "Teachers", "Admins", "Responsables"}
    );
    ...
    // Buttons
    JPanel buttonPanel = new JPanel(new FlowLayout());
    JButton sendButton = createStyledButton("📤 Send Announcement", Color.WHITE, LIGHT_BLUE);
    sendButton.addActionListener(e -> {
        sendAnnouncement((String)recipientCombo.getSelectedItem(), 
                        subjectField.getText(), contentArea.getText(), dialog);
    });
...
private void sendAnnouncement(String recipient, String subject, String content, JDialog dialog) {
    ...
    switch (recipient) {
        case "All Users":
            success = adminController.sendAnnouncementToAll(subject, content);
            break;
        case "Students":
            success = adminController.broadcastToUserType("STUDENT", subject, content);
            break;
        case "Teachers":
            success = adminController.broadcastToUserType("TEACHER", subject, content);
            break;
        case "Admins":
            success = adminController.broadcastToUserType("ADMIN", subject, content);
            break;
        case "Responsables":
            success = adminController.broadcastToUserType("RESPONSABLE", subject, content);
            break;
    }

...
private void debugTeacherSetup() {
  ...
  private JPanel createGradesTab() {
    ...
    JPanel formPanel = new JPanel(new GridBagLayout());
    formPanel.setBorder(BorderFactory.createTitledBorder("Add Grades"));
...
private JPanel createMessagingTab() {
    ...
    sendMessageButton = createStyledButton("📤 Send Message to Students", Color.WHITE, new Color(33, 150, 243));
...
private void initializeMessaging() {
    ...
    int teacherId = Integer.parseInt(userInfo.get("teacherId"));
    clientService.getMessageService().subscribeToUserMessages(teacherId, "TEACHER", listener);
    
    // Also subscribe to broadcasts for teachers
    clientService.getMessageService().subscribeToBroadcast("TEACHER", listener);
}
...
private void showSendMessageDialog() {
    ...
    loadTeacherSubjectsToMessageCombo(subjectCombo);
    recipientPanel.add(new JLabel("Subject:"));
    recipientPanel.add(subjectCombo);
    ...
    subjectField.setText("Grade Announcement");
    ...
    String[] studentColumns = {"Select", "Student Name", "Program"};
    ...
    sendButton.addActionListener(e -> {
        sendMessageToStudents(subjectCombo, subjectField, contentArea, studentsTable, 
...
private void loadTeacherSubjectsToMessageCombo(JComboBox<String> comboBox) {
    try {
        int teacherId = Integer.parseInt(userInfo.get("teacherId"));
        List<Map<String, String>> subjects = clientService.getTeacherSubjects(teacherId);
        comboBox.removeAllItems();
        ...
        for (Map<String, String> subject : subjects) {
            comboBox.addItem(subject.get("subjectName"));
        }
    } catch (Exception e) {
        JOptionPane.showMessageDialog(this, "Error loading subjects: " + e.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
    }
...
private void sendMessageToStudents(JComboBox<String> subjectCombo, JTextField subjectField, 
        ...
        if (selectSpecific) {
            ...
            for (int i = 0; i < model.getRowCount(); i++) {
                ...
                if (isSelected != null && isSelected) {
                    // Extract student ID from the student name
                    String studentInfo = (String) model.getValueAt(i, 1);
                    String studentId = extractIdFromSelection(studentInfo);
                    if (studentId != null) {
            ...
            success = clientService.teacherSendMessageToStudents(teacherId, studentIds, subject, content);
            
        } else {
            // Send to all students in the selected subject
            if ("All Subjects".equals(selectedSubject)) {
                // Send to all students of this teacher
                List<Map<String, String>> allStudents = clientService.getTeacherStudents(teacherId);
                List<Integer> studentIds = new ArrayList<>();
                
                for (Map<String, String> student : allStudents) {
                    String studentId = extractIdFromSelection(student.get("studentName") + " (" + student.get("studentId") + ")");
                    if (studentId != null) {
                        studentIds.add(Integer.parseInt(studentId));
                    }
                }
                ...
                // Send to students in specific subject
                Integer subjectId = clientService.getSubjectIdByName(selectedSubject);
                if (subjectId != null) {
                    List<Map<String, String>> subjectStudents = clientService.getStudentsBySubjectForTeacher(teacherId, subjectId);
                    List<Integer> studentIds = new ArrayList<>();
                    
                    for (Map<String, String> student : subjectStudents) {

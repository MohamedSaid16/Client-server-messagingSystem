...
public class TeacherController {
   ...
   public boolean updateExam(int examId, String examName, double coefficient) {
    ...
    public boolean sendMessageToStudent(int teacherId, int studentId, String subject, String content) {
    return clientService.sendMessage(studentId, "STUDENT", "MESSAGE", subject, content);
}

public boolean sendMessageToStudents(int teacherId, List<Integer> studentIds, String subject, String content) {
    return clientService.teacherSendMessageToStudents(teacherId, studentIds, subject, content);
}

public boolean sendMessageToClass(int teacherId, int subjectId, String subject, String content) {
    // Get all students in the subject
    List<Map<String, String>> students = clientService.getStudentsBySubjectForTeacher(teacherId, subjectId);
    List<Integer> studentIds = new ArrayList<>();
    for (Map<String, String> student : students) {
        studentIds.add(Integer.parseInt(student.get("studentId")));
    }
    return sendMessageToStudents(teacherId, studentIds, subject, content);
}

// Grade notifications
public boolean notifyStudentsAboutGrade(int subjectId, String examName) {
    List<Map<String, String>> students = clientService.getStudentsBySubjectForTeacher(-1, subjectId);
    boolean allSent = true;
    for (Map<String, String> student : students) {
        int studentId = Integer.parseInt(student.get("studentId"));
        boolean sent = clientService.sendNotification(studentId, "STUDENT", 
            "Grade Published", 
            String.format("Your grade for %s has been published", examName), 
            "GRADE");
...
public void subscribeToTeacherMessages(MessageService.MessageListener listener, int teacherId) {

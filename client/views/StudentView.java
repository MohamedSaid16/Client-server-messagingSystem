...
public StudentView(StudentController studentController, Map<String, String> userInfo) {
    ...
    this.receivedMessages = new ArrayList<>(); // Initialize the list
    
    // Extract student ID from userInfo or controller
    this.currentStudentId = Integer.parseInt(userInfo.get("userId"));
    
    initializeUI();
    loadStudentData();
    
    // CORRECT WAY: Add message listener first, then subscribe
    studentController.addMessageListener(new MessageService.MessageListener() {
        @Override
    ...
    // THEN subscribe to messages with just the student ID
    studentController.subscribeToStudentMessages(currentStudentId);
}
    ...
    private void initializeUI() {
        ...
        tabbedPane.addTab("✉️ Messages", createMessagingTab());
...
private JPanel createMessagingTab() {
    ...
    sendMessageButton = createStyledButton("📤 Send Message to Teacher", Color.WHITE, new Color(33, 150, 243));
...
private void initializeMessaging() {
    ...
    int studentId = Integer.parseInt(userInfo.get("studentId"));
    studentController.subscribeToStudentMessages(studentId);
}
...
private void showSendMessageDialog() {
    JDialog dialog = new JDialog(this, "Send Message to Teacher", true);
    ...
    teacherPanel.add(new JLabel("Send to Teacher:"));
    JComboBox<String> teacherCombo = new JComboBox<>();
    loadTeachersToComboBox(teacherCombo);
    ...
    subjectField.setText("Question about course");
    ...
    formPanel.add(teacherPanel, BorderLayout.NORTH);
    ...
    sendButton.addActionListener(e -> {
        sendMessageToTeacher(teacherCombo, subjectField, contentArea, dialog);
    });
...
private void loadTeachersToComboBox(JComboBox<String> comboBox) {
    try {
        int studentId = Integer.parseInt(userInfo.get("studentId"));
        ...
        if (subjects != null && !subjects.isEmpty()) {
            // For each subject, get the teacher
            for (Map<String, String> subject : subjects) {
                String subjectName = subject.get("subjectName");
                // In a real implementation, you would get the teacher for this subject
                // For now, we'll create a placeholder
                comboBox.addItem("Teacher for " + subjectName);
            }
        } else {
            comboBox.addItem("No teachers available - contact administration");
        }
        
        // Add generic options
        comboBox.addItem("My Program Coordinator");
        comboBox.addItem("Academic Advisor");
        
    } catch (Exception e) {
        JOptionPane.showMessageDialog(this, "Error loading teachers: " + e.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
    }
}
private void sendMessageToTeacher(JComboBox<String> teacherCombo, JTextField subjectField, 
                                JTextArea contentArea, JDialog dialog) {
    ...
    if (selectedTeacher == null || selectedTeacher.contains("No teachers available")) {
        JOptionPane.showMessageDialog(this, 
            "No teachers available for messaging.\nPlease contact administration directly.", 
            "No Teachers", JOptionPane.WARNING_MESSAGE);
        return;
    }
    ...
    try {
        ...
        // In a real implementation, you would get the actual teacher ID
        // For now, we'll use a placeholder and show success
        int teacherId = 1; // This should come from your database
        
        boolean success = studentController.sendMessageToTeacher(studentId, teacherId, subject, content);
        
        if (success) {
            JOptionPane.showMessageDialog(this, 
                "Message sent successfully to " + selectedTeacher + "!", 
...
private void refreshMessageDisplay() {
    // Update your message UI components here
    if (messageArea != null) {
        StringBuilder sb = new StringBuilder();
        for (Message msg : receivedMessages) {
            sb.append("From: ").append(msg.getSenderType())
              .append(" ").append(msg.getSenderId())

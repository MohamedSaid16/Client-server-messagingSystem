...
public boolean sendAnnouncementToAll(String subject, String content) {
    try {
        return clientService.sendSystemAnnouncement(subject, content);
    } catch (Exception e) {
...
public boolean sendMessageToUser(int userId, String userType, String subject, String content) {
    try {
        return clientService.sendMessage(userId, userType, "MESSAGE", subject, content);
    } catch (Exception e) {
...
public boolean broadcastToUserType(String userType, String subject, String content) {
    try {
        return clientService.broadcastToUserType(userType, subject, content);
    } catch (Exception e) {
...
public boolean sendSystemAlert(String title, String message) {
    try {
        return clientService.broadcastToUserType("ALL", title, message);
    } catch (Exception e) {
...
public void subscribeToAdminMessages(MessageService.MessageListener listener, int adminId) {
    try {
        MessageService messageService = clientService.getMessageService();
        if (messageService != null && messageService.isInitialized()) {
            // Subscribe to personal admin messages
            messageService.subscribeToUserMessages(adminId, "admin", listener);
            
            // Subscribe to admin broadcasts
            messageService.subscribeToBroadcast("admin", listener);
            
            System.out.println("✅ Admin " + adminId + " subscribed to messages successfully");
        } else {
            System.err.println("❌ MessageService not available for subscription");
        }
    } catch (Exception e) {
        System.err.println("❌ Error subscribing admin to messages: " + e.getMessage());
        e.printStackTrace();
    }
}
public MessageService getMessageService() {
    return clientService.getMessageService();
}

// Debug messaging system
public void debugMessagingSystem() {
    try {
        MessageService messageService = clientService.getMessageService();
        if (messageService != null) {
            messageService.debugQueuesAndTopics();
        } else {
            System.err.println("❌ MessageService is null");
        }
    } catch (Exception e) {
        System.err.println("❌ Error debugging messaging system: " + e.getMessage());
    }
}
}
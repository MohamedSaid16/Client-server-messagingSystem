...
private JTextArea messageArea;
    ...
    private void initializeUI() {
        ...
        tabbedPane.addTab("✉️ Messaging", createMessagingTab());
...
boolean success = responsableController.addStudent(firstName, lastName, email, phone, schoolOrigin);
...
private JPanel createMessagingTab() {
    JPanel panel = new JPanel(new BorderLayout());
    ...
    messageScroll.setBorder(BorderFactory.createTitledBorder("📨 Received Messages"));
    ...
    JButton sendMessageButton = createStyledButton("📤 Send Message to Students", Color.WHITE, new Color(33, 150, 243));
    sendMessageButton.addActionListener(e -> showSendMessageDialog());
    quickActionPanel.add(sendMessageButton);
    
    JButton clearMessagesButton = createStyledButton("🗑️ Clear Messages", Color.WHITE, new Color(255, 152, 0));
    clearMessagesButton.addActionListener(e -> clearMessages(messageArea));
    quickActionPanel.add(clearMessagesButton);
    
    JButton refreshMessagesButton = createStyledButton("🔄 Refresh Messages", Color.WHITE, new Color(76, 175, 80));
    refreshMessagesButton.addActionListener(e -> refreshMessages(messageArea));
    quickActionPanel.add(refreshMessagesButton);
    
    // Message type filter
    JPanel filterPanel = new JPanel(new FlowLayout());
    filterPanel.add(new JLabel("Filter:"));
    JComboBox<String> filterCombo = new JComboBox<>(new String[]{"All", "Announcements", "Alerts", "Notifications"});
    filterCombo.addActionListener(e -> filterMessages((String)filterCombo.getSelectedItem(), messageArea));
    filterPanel.add(filterCombo);
    
    controlPanel.add(quickActionPanel);
    ...
    initializeMessaging(messageArea);
...
private void initializeMessaging(JTextArea messageArea) {
    receivedMessages = new ArrayList<>();
    
    // Subscribe to responsable messages
    MessageService.MessageListener listener = new MessageService.MessageListener() {
        @Override
        public void onMessageReceived(Message message) {
            SwingUtilities.invokeLater(() -> {
                receivedMessages.add(message);
                displayMessage(message, messageArea);
                showMessageNotification(message);
            });
        }
    };
    
    int responsableId = Integer.parseInt(userInfo.get("userId"));
    responsableController.subscribeToResponsableMessages(listener, responsableId);
    
    // Also subscribe to broadcasts for responsables
    responsableController.subscribeToBroadcast("RESPONSABLE", listener);
}

private void displayMessage(Message message, JTextArea messageArea) {
...
private void clearMessages(JTextArea messageArea) {
    receivedMessages.clear();
    messageArea.setText("");
}

private void refreshMessages(JTextArea messageArea) {
    // Could reload messages from database if needed
    JOptionPane.showMessageDialog(this,
        "Messages refreshed!",
        "Refresh",
        JOptionPane.INFORMATION_MESSAGE);
...
private void filterMessages(String filter, JTextArea messageArea) {
    messageArea.setText("");
    for (Message message : receivedMessages) {
        if (filter.equals("All") || message.getMessageType().equalsIgnoreCase(filter)) {
            displayMessage(message, messageArea);
        }
    }
}
private void showSendMessageDialog() {
    JDialog dialog = new JDialog(this, "Send Message to Students", true);
    dialog.setLayout(new BorderLayout());
    ...
    // Program selection
    JComboBox<String> programCombo = new JComboBox<>();
    loadProgramsToMessageCombo(programCombo);
    recipientPanel.add(new JLabel("Program:"));
    recipientPanel.add(programCombo);
    
    // All students or specific selection
    JCheckBox selectStudentsCheckbox = new JCheckBox("Select specific students");
    recipientPanel.add(selectStudentsCheckbox);
    
    // Subject field
    JPanel subjectPanel = new JPanel(new BorderLayout());
    subjectPanel.add(new JLabel("Message Subject:"), BorderLayout.NORTH);
    JTextField subjectField = new JTextField();
    subjectField.setText("Academic Announcement");
    ...
    // Student selection panel (initially hidden)
    JPanel studentSelectionPanel = new JPanel(new BorderLayout());
    studentSelectionPanel.setBorder(BorderFactory.createTitledBorder("Select Students"));
    
    String[] studentColumns = {"Select", "Student Name", "Program", "Academic Year"};
    ...
    loadStudentsToMessageTable(studentModel);
    
    JScrollPane studentScroll = new JScrollPane(studentsTable);
    studentSelectionPanel.add(studentScroll, BorderLayout.CENTER);
    studentSelectionPanel.setVisible(false);
    
    // Toggle student selection visibility
    selectStudentsCheckbox.addActionListener(e -> {
        studentSelectionPanel.setVisible(selectStudentsCheckbox.isSelected());
        dialog.pack();
    });
    
    formPanel.add(recipientPanel, BorderLayout.NORTH);
    formPanel.add(subjectPanel, BorderLayout.CENTER);
    formPanel.add(contentPanel, BorderLayout.SOUTH);
    
    // Buttons
    JPanel buttonPanel = new JPanel(new FlowLayout());
    JButton sendButton = createStyledButton("📤 Send Message", Color.WHITE, new Color(76, 175, 80));
    sendButton.addActionListener(e -> {
        sendMessageToStudents(programCombo, subjectField, contentArea, studentsTable, 
                            selectStudentsCheckbox.isSelected(), dialog);
    });
    
    JButton cancelButton = createStyledButton("❌ Cancel", Color.WHITE, new Color(244, 67, 54));
    ...
    mainPanel.add(studentSelectionPanel, BorderLayout.CENTER);
...
private void loadProgramsToMessageCombo(JComboBox<String> comboBox) {
    try {
        List<Map<String, String>> programs = responsableController.getAllPrograms();
        comboBox.removeAllItems();
        comboBox.addItem("All Programs");
        for (Map<String, String> program : programs) {
            comboBox.addItem(program.get("programName"));
        }
    } catch (Exception e) {
        JOptionPane.showMessageDialog(this, "Error loading programs: " + e.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
    }
...
private void loadStudentsToMessageTable(DefaultTableModel model) {
    try {
        List<Map<String, String>> students = responsableController.getAllStudents();
        model.setRowCount(0);
        ...
        for (Map<String, String> student : students) {
            model.addRow(new Object[]{
                ...
                student.get("firstName") + " " + student.get("lastName"),
                student.get("program") != null ? student.get("program") : "Not assigned",
                student.get("academicYear") != null ? student.get("academicYear") : "Not set"
            });
        }
    } catch (Exception e) {
        JOptionPane.showMessageDialog(this, "Error loading students: " + e.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
...
private void sendMessageToStudents(JComboBox<String> programCombo, JTextField subjectField, 
                                 JTextArea contentArea, JTable studentsTable, 
                                 boolean selectSpecific, JDialog dialog) {
    String subject = subjectField.getText().trim();
    String content = contentArea.getText().trim();
    String selectedProgram = (String) programCombo.getSelectedItem();
    
    if (subject.isEmpty() || content.isEmpty()) {
        JOptionPane.showMessageDialog(this, "Please fill subject and content", "Error", JOptionPane.ERROR_MESSAGE);
        return;
    }
    ...
    try {
        ...
        if (selectSpecific) {
            ...
            for (int i = 0; i < model.getRowCount(); i++) {
                ...
                if (isSelected != null && isSelected) {
                    // Get student ID from the student list
                    List<Map<String, String>> allStudents = responsableController.getAllStudents();
            ...
            success = responsableController.sendMessageToStudents(responsableId, studentIds, subject, content);
            
        } else {
            // Send to all students in the selected program
            if ("All Programs".equals(selectedProgram)) {
                ...
                Integer programId = responsableController.getProgramIdByName(selectedProgram);
                if (programId != null) {
                    List<Map<String, String>> programStudents = responsableController.getStudentsByProgram(programId);
                    List<Integer> studentIds = new ArrayList<>();
        ...
        if (success) {
            JOptionPane.showMessageDialog(this, 
                "Message sent successfully to students!", 
                "Success", JOptionPane.INFORMATION_MESSAGE);
            dialog.dispose();
        } else {
            JOptionPane.showMessageDialog(this, 
                "Failed to send message to some students", 
